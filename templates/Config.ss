...
<Piecemaker>
  <Contents>
		<% control PMItems %>
			...
			<% control PMItemFile %>
				<% if Extension == mp4 %>
				    <Video Source="/$FileName" Title="$Top.AlternateText" Width="940" Height="400" Autoplay="true">
				...
				<% end_if %>
				<% if Extension == swf %>				
				    <Flash Source="/$FileName" Title="$Top.AlternateText">
				      <Image Source="PieceMaker/contents/flash-preview.png" />
				    </Flash>
				<% end_if %>					

				<% if Extension == jpg %>
	   				<Image Source="<% control CroppedImage(940,400) %>$AbsoluteURL<% end_control %>">
						<Text>$Top.AlternateText</Text>
					</Image>				
				<% end_if %>				
				<% if Extension == png %>				
	   				<Image Source="<% control CroppedImage(940,400) %>$AbsoluteURL<% end_control %>" Title="$AlternateText"></Image>				
				<% end_if %>				
